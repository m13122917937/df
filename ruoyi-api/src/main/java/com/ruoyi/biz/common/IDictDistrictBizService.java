...
public class IDictDistrictBizService {
    ...
    public List<DictDistrictBO> listProvince() {
        ...
        List<DictDistrictBO> list = redisCache.getCacheObject(RedisKeyUtil.generate(ApiRedisKey.API_BASE, SysRedisKey.ROOT_KEY, "province"));
        if (CollectionUtil.isEmpty(list)) {
            list = dictDistrictFacade.list(new DictDistrictQuery().setLevel(DictDistrictConsts.Level.PROVINCE.getValue()));
        }
        redisCache.setCacheObject(RedisKeyUtil.generate(ApiRedisKey.API_BASE, SysRedisKey.ROOT_KEY, "province"), list, 1, TimeUnit.DAYS);
        return list;
    }

    public List<DictDistrictBO> listCity(final Long province) {

        List<DictDistrictBO> list = redisCache.getCacheObject(RedisKeyUtil.generate(ApiRedisKey.API_BASE, SysRedisKey.ROOT_KEY, "city", String.valueOf(province)));
        if (CollectionUtil.isEmpty(list)) {
            list = dictDistrictFacade.list(new DictDistrictQuery().setLevel(DictDistrictConsts.Level.CITY.getValue()).setPid(province));
        }

        redisCache.setCacheObject(RedisKeyUtil.generate(ApiRedisKey.API_BASE, SysRedisKey.ROOT_KEY, "city", String.valueOf(province)), list, 1, TimeUnit.DAYS);
        return list;
    }
}
