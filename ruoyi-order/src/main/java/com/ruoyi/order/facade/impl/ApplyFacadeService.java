...
import com.ruoyi.order.manager.ApplyManager;
...
@Slf4j
@Service
public class ApplyFacadeService implements IApplyFacade {
    ...
    public List<ApplyBO> list(ApplyQuery query) {
        ...
        return ApplyCov.INSTANCE.listToBO(applyManager.list(DynamicCondition.toWrapper(query)));
    }
    ...
    public PageBO<ApplyBO> listPage(final ApplyQuery query, final PageParamV2 pageParam) {
        ...
        return PageUtils.fromList(applyManager.list(wrapper), ApplyCov.INSTANCE::listToBO);
    }


    @Override
    public ApplyBO getOne(ApplyQuery query) {
        return ApplyCov.INSTANCE.toBO(applyManager.getOne(DynamicCondition.toWrapper(query)));
    }

    @Override
    public boolean update(ApplyParam param, ApplyQuery query) {
        return applyManager.update(ApplyCov.INSTANCE.paramToDomain(param), DynamicCondition.toWrapper(query));
    }

    @Override
    public boolean remove(ApplyQuery query) {
        return applyManager.remove(DynamicCondition.toWrapper(query));
    }
