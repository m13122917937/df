...
import com.ruoyi.rule.service.RuleService;
 ...
 */
@Service
public class RuleFacade implements IRuleFacade {
    ...
    public List<RuleBO> list(RuleQuery query) {
        ...
        return RuleCov.INSTANCE.listToBO(ruleService.list(wrapper));
    }

    @Override
    public PageBO<RuleBO> listPage(RuleQuery query, PageParamV2 pageParam) {
        PageHelper.startPage(pageParam.getPage(), pageParam.getSize());
        List<Rule> list = ruleService.list(DynamicCondition.toWrapper(query, pageParam.getSort()));
        return PageUtils.fromList(list, RuleCov.INSTANCE::listToBO);
    }

    @Override
    public RuleBO getOne(RuleQuery query) {
        return RuleCov.INSTANCE.toBO(ruleService.getOne(DynamicCondition.toWrapper(query)));
    }

    @Override
    public boolean update(RuleParam param, RuleQuery query) {
        return ruleService.update(RuleCov.INSTANCE.paramToDomain(param), DynamicCondition.toWrapper(query));
    }
