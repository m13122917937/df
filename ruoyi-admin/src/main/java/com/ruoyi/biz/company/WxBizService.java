...
public class WxBizService {
    ...
    public void pushUser(WxMpUser userWxInfo, String eventKey) {
        UserBO userBO = userFacade.queryOne(new UserQuery().setOpenId(userWxInfo.getOpenId()).setUnionId(userWxInfo.getUnionId()));
        ...
        if (Objects.equals(Convert.toInt(type), UserApiConsts.INVITATION_SUB_QR_CODE)) {
            if (Objects.isNull(userBO)) {
                // 添加用户 并且添加 企业用户信息
                userFacade.addUserAndCompany(new UserParam().setUnionId(userWxInfo.getUnionId()).setOpenId(userWxInfo.getOpenId())
                        .setPhone(map.get(UserApiConsts.QR_CODE_SCENE_PHONE)).setNickName(map.get(UserApiConsts.QR_CODE_SCENE_USERNAME))
                ...
                userFacade.addUserCompany(userBO.getUserId(), Convert.toLong(map.get(UserApiConsts.QR_CODE_SCENE_DEPTID)), name);
            }
            ...
            userLoginBO.setUserId(userBO.getUserId()).setNewUser(Boolean.FALSE);
            // 登录成功设置key
            redisCache.setCacheObject(UserRedisKey.USER_KEY.USER_LOGIN_KEY.getKey() + uuid, userLoginBO, 30, TimeUnit.MINUTES);
        }
