...
import com.ruoyi.capital.manager.CompanyCapitalLogManager;
import com.ruoyi.capital.manager.CompanyCapitalManager;
...
@Slf4j
@Service
public class CompanyCapitalFacadeService implements ICompanyCapitalFacade {

    @Autowired
    CompanyCapitalManager companyCapitalManager;


    @Autowired
    CompanyCapitalLogManager companyCapitalLogManager;


    @Override
    public CompanyCapitalBO queryOne(CompanyCapitalQuery query) {

        CompanyCapital companyCapital = companyCapitalManager.getOne(DynamicCondition.toWrapper(query));
    ...
    public void changeAvailable(final CompanyCapitalLogParam logParam) {
        CompanyCapital companyCapital = companyCapitalManager.queryOrInit(CompanyCapitalConsts.Types.DEPOSIT.getCode(), logParam.getCompanyId());
        // 增加资金记录
        ...
        if (!companyCapitalLogManager.save(CompanyCapitalLogCov.INSTANCE.toDomain(logParam))) {
            return;
        }
        // 修改资金账号
        companyCapitalManager.updateAmount(companyCapital, logParam.getAddAmount());
    }
    ...
    public void freeze(CompanyCapitalLogParam logParam) {
        ...
        CompanyCapitalLog companyCapitalLog = companyCapitalLogManager.getOne(DynamicCondition.toWrapper(new CompanyCapitalLogQuery()
        ...
        companyCapitalLogManager.save(companyCapitalLog);

        boolean b = companyCapitalManager.updateFrozenAmount(companyCapital, Arith.add(logParam.getAddAmount(), logParam.getOutAmount()));
        if (!b) {
    ...
    public void unFreeze(CompanyCapitalLogParam logParam) {
        ...
        companyCapitalLogManager.update(companyCapitalLog, new LambdaQueryWrapper<>(CompanyCapitalLog.class).eq(CompanyCapitalLog::getId, companyCapitalLog.getId()));
        // 修改资金账号
        boolean update = companyCapitalManager.updateFrozenAmount(companyCapital, Arith.add(logParam.getAddAmount(), logParam.getOutAmount()));
        if (!update) {
            throw new ServiceException("修改资金账号失败", HttpStatus.CAPITAL_WARN);
        }
    }
    ...
    public PageBO<CompanyCapitalLogBO> pageLog(CompanyCapitalLogQuery query, PageParamV2 pageParamV2) {
        ...
        List<CompanyCapitalLog> list = companyCapitalLogManager.list(DynamicCondition.toWrapper(query));
