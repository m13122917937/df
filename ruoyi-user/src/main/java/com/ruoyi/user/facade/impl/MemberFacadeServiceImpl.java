...
import com.ruoyi.user.manager.MemberCompanyManager;
import com.ruoyi.user.manager.MemberManager;
...
@Slf4j
@Service
public class MemberFacadeServiceImpl implements IMemberFacade {
    ...
    @Autowired
    MemberCompanyManager memberCompanyManager;
    ...
    public List<MemberBO> queryList(final MemberQuery memberQuery) {
        ...
        List<Member> list = memberManager.list(new QueryWrapper<>(member));
        return MemberCov.INSTANCE.domainToBoList(list);
    }

    @Override
    public MemberBO queryOne(final MemberQuery memberQuery) {
        return MemberCov.INSTANCE.domainToBo(memberManager.getOne(new QueryWrapper<>(MemberCov.INSTANCE.queryToBo(memberQuery))));
    }
    ...
    public MemberBO addMemberAndCompany(final MemberParam memberParam) {
        ...
        memberCompanyManager.save(memberCompany);
        return memberBO;
    }
    ...
    public PageBO<MemberBO> memberList(final MemberQuery memberQuery, final PageParamV2 pageParamV2) {
        ...
        List<CompanyUserDTO> companyUsers = memberManager.companyUser(memberQuery);
    ...
    public void addMemberCompany(Long memberId, Long companyId, String name) {
        long count = memberCompanyManager.count(DynamicCondition.toWrapper(new MemberCompanyQuery().setUserId(memberId).setCompanyId(companyId)));
        if (count > 1) {
            return;
        }
        ...
        memberManager.update(new Member().setNickName(name), DynamicCondition.toWrapper(new MemberQuery().setUserId(memberId)));
    }
    ...
    public PageBO<MemberBO> pageCompanyMember(MemberQuery memberQuery, PageParamV2 pageParamV2) {
        ...
        PageUtils.startPage(pageParamV2);
        List<MemberCompany> memberCompanyList = memberCompanyManager.list(DynamicCondition.toWrapper(new MemberCompanyQuery().setCompanyId(memberQuery.getCompanyId())));
        ...
        List<Member> memberList = memberManager.list(DynamicCondition.toWrapper(new MemberQuery().setUserIdSet(new ArrayList<>(userIdSet))));
    ...
    public boolean companyMasterUser(Long companyId, Long currentUserId) {
        long count = memberCompanyManager.count(DynamicCondition.toWrapper(new MemberCompanyQuery().setCompanyId(companyId).setUserId(currentUserId).setOwner(MemberEnum.UserOwner.MASTER.getValue())));
        return count > 0;
    }
