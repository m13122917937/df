...
import com.ruoyi.product.manager.ProductSkuManager;
...
@Slf4j
@Service
public class ProductSkuFacadeService implements IProductSkuFacade {
    ...
    public List<ProductSkuBO> list(ProductSkuQuery query, SortBy sort) {
        return ProductSkuCov.INSTANCE.listToBO(productSkuManager.list(DynamicCondition.toWrapper(query,sort)));
    }

    @Override
    public PageBO<ProductSkuBO> pageList(ProductSkuQuery query, PageParamV2 pageParamV2) {
        PageUtils.startPage(pageParamV2);
        List<ProductSku> list = productSkuManager.list(DynamicCondition.toWrapper(query, pageParamV2.getSort()));
        return PageUtils.fromList(list, ProductSkuCov.INSTANCE::listToBO);
    }

    @Override
    public ProductSkuBO getOne(ProductSkuQuery query) {
        return ProductSkuCov.INSTANCE.toBO(productSkuManager.getOne(DynamicCondition.toWrapper(query)));
    }
    ...
    public boolean update(ProductSkuParam param, ProductSkuQuery query) {
        ...
        return productSkuManager.update(ProductSkuCov.INSTANCE.paramToDomain(param), new QueryWrapper<>(queryDomain));
    }
    ...
    public ProductSkuBO save(ProductSkuParam param) {
        ...
        boolean save = productSkuManager.save(productSku);
    ...
    public long count(ProductSkuQuery productSkuQuery) {
         return productSkuManager.count(DynamicCondition.toWrapper(productSkuQuery));
    }
