 ...
 * <p>抖音脱敏规则：
 * <ul>
 *   <li>电话加密格式：$$...=$1$$（长格式加密）</li>
 *   <li>姓名加密格式：##...#1##（长格式加密）</li>
 *   <li>地址拆分：province, city, district, addr（包含四级地址+详细地址密文）</li>
...
@Slf4j
@Component
public class DouyinDesensitizedOrderStrategy extends AbstractDesensitizedOrderStrategy {

    @Override
    public EOrderRequestParam buildOrderParam(DesensitizedOrderRequest request) {
        log.info("【抖音脱敏订单】开始构建订单参数，平台订单ID: {}", request.getThirdOrderId());
    ...
    private void addPlatformSpecificParams(EOrderRequestParam param, PlatformAuthInfo authInfo, String thirdOrderId) {
        ...
        log.debug("【抖音脱敏订单】平台参数 - partnerId: {}, thirdOrderId: {}",
                authInfo.getPartnerId(), thirdOrderId);
    }

    @Override
    public String getPlatformCode() {
        return PlatformType.DOUYIN.getCode();
    }
}