 ...
 *   <li>电话加密格式：$...=$1$$</li>
 *   <li>姓名加密格式：#...#1##</li>
 *   <li>地址拆分：province, city, district, street, addr（详细地址密文）</li>
 *   <li>需要 detailAddrEnc（加密详细地址，格式：~...=~1~~）</li>
 * </ul>
 *
 * @author kuaidi100
 */
@Slf4j
@Component
public class KuaishouDesensitizedOrderStrategy extends AbstractDesensitizedOrderStrategy {

    @Override
    public EOrderRequestParam buildOrderParam(DesensitizedOrderRequest request) {
        log.info("【快手脱敏订单】开始构建订单参数，平台订单ID: {}", request.getThirdOrderId());
    ...
    private void addPlatformSpecificParams(EOrderRequestParam param, PlatformAuthInfo authInfo, String thirdOrderId) {
        ...
        log.debug("【快手脱敏订单】平台参数 - partnerId: {}, thirdOrderId: {}",
                authInfo.getPartnerId(), thirdOrderId);
    }

    @Override
    public String getPlatformCode() {
        return PlatformType.KUAISHOU.getCode();
    }
}