  ...
  <div class="order-detail refund-order">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <!-- <span class="page-subtitle">管理退货追单订单，处理发货流程</span> -->
      </div>
      ...
      <div class="order-table-container-wrapper">
        ...
        <div class="order-table-container">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <span>{{ formatSubStatus(scope.row.subStatus) }}</span>
              </template>
            </el-table-column>
            ...
            <!-- 退货追单相关列 -->
            <el-table-column prop="confirmTime" label="确认追单时间" width="200" align="center">
              <template slot-scope="scope">
            ...
            <el-table-column label="操作" width="300" fixed="right" align="center">
              <template slot-scope="scope">
                <div class="operation-buttons">
                  <el-button size="mini" type="primary" @click="handleImeiInfo(scope.row)">串码信息</el-button>
                  <el-button
                    v-if="scope.row.subStatus !== 71 && scope.row.subStatus !== 72"
                    size="mini"
                    type="warning"
...
export default {
  ...
  methods: {
    ...
    // 确认追单方法
    handleOperation(row) {
      console.log('确认追单:', row)
      this.$confirm('确认要追单此订单吗？', '确认追单', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async() => {
        const res = await confirmChase(row.orderCode)
        if (res && res.code === 200) {
          this.$message.success('确认追单成功')
          this.fetchOrderList()
        } else {
          this.$message.error(res?.msg || '确认追单失败，请稍后重试')
        }
      }).catch(() => {
        this.$message.info('已取消追单')
      })
    },
    // 处理地区变更
    ...
    formatRevokeReason(revokeType) {
      if (revokeType === null || revokeType === undefined || revokeType === '') {
        return '-'
      }
      ...
      return reasonMap[revokeType] || `未知原因(${revokeType})`
    },
    // 格式化子状态
    formatSubStatus(subStatus) {
      if (subStatus === null || subStatus === undefined || subStatus === '') {
        return '-'
      }
      ...
      return statusMap[subStatus] || `未知状态(${subStatus})`
    }
  }
}
</script>
