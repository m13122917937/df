...
// RECEIPT(9, "待确定收货"),
// ENDING(10, "已完成"),
import { getBrandCountApi } from '@/api/order'
export default {
  name: 'BrandFilter',
  props: {
    status: {
    ...
    async getBrandData() {
      ...
      try {
        ...
        if (res && res.code === 200) {
          ...
          console.log('处理后的品牌数据:', this.brandData)
        } else {
...
.brand-btn.is-primary {
  ...
  &:hover {
    background: linear-gradient(135deg, #3395FF 0%, #1677FF 100%);
  }
}
