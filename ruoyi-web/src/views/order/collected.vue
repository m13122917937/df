<!--确认收货-->
<template>
  <div class="order-detail collected-order">
    <div class="page-banner">
      <div class="banner-inner">
        <div class="banner-top">
          <div class="banner-info">
            <h2 class="banner-title"><i class="el-icon-s-order" />确认收货订单管理</h2>
            <p class="banner-desc">管理确认收货订单，处理签收流程</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="order-main">
        ...
        <div class="table-card">
          <el-table ref="table" v-loading="loading" :data="orderList" stripe size="medium" style="width: 100%" :fit="true" height="100%">
            <template slot="empty"><EmptyState text="暂无确认收货数据" /></template>
            ...
            <el-table-column prop="confirmTime" label="确认收货时间" width="180" align="center"><template slot-scope="scope">{{ scope.row.confirmTime || '-' }}</template></el-table-column>
...
export default {
  ...
  data() {
    return {
      loading: false, orderList: [], selectedRegion: '', selectedBrand: '', currentStatus: 9,
  ...
  methods: {
    ...
    async fetchOrderList() {
      ...
      const res = await getConfirmList(params, { pageNum: this.pagination.current, pageSize: this.pagination.size })
