      ...
      <div class="banner-inner">
        <div class="banner-top">
          <div class="banner-info">
            <h2 class="banner-title"><i class="el-icon-s-order" />在途订单管理</h2>
            <p class="banner-desc">管理在途订单，跟踪物流状态</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="order-main">
        ...
        <div class="table-card">
          <el-table ref="table" v-loading="loading" :data="orderList" stripe size="medium" style="width: 100%" :fit="true" height="100%">
            <template slot="empty"><EmptyState text="暂无在途数据" /></template>
            ...
            <el-table-column label="操作" width="160" fixed="right" align="center">
              <template slot-scope="scope">
                <div class="operation-buttons">
                  <el-button v-if="scope.row.orderType !== 1" size="small" class="op-btn" @click="handleImeiInfo(scope.row)">串码信息</el-button>
                </div>
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  data() {
    return {
      loading: false, orderList: [], selectedRegion: '', selectedBrand: '', currentStatus: 6,
  ...
  methods: {
    ...
    async fetchOrderList() {
      ...
      const res = await getTransitList(params, { pageNum: this.pagination.current, pageSize: this.pagination.size })
