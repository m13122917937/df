<template>
  <div class="shipped-orders orders-wrap">
    <BrandFilter
      :status="status"
      :current-brand="brand"
...
import { apiGetDeliveryExport } from "@/api/creatingOrders";
  ...
  methods: {
    ...
    buildQueryParams() {
      return {
        ...
        orderCodeList: this.orderCode ? [this.orderCode] : [],
        originalOrderId: this.originalOrderId,
        category: this.category,
        shopName: this.shopName,
        productNameLike: this.productNameLike,
        skuNameLike: this.skuNameLike,
    ...
    async handleExport() {
      ...
      apiGetDeliveryExport(params,pageData).then((res) => {
        const isBlob = blobValidate(res);
        if (isBlob) {
          const blob = new Blob([res]);
          saveAs(blob, `当日发货订单_${date}.xlsx`);
