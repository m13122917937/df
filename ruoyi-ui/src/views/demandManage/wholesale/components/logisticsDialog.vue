    ...
    <div class="logistics-info-section">
      <div class="info-header">
        <div class="express-type">
          ...
          <span>{{ (currentOrder && currentOrder.trackingCompany) || '快递发货' }}</span>
        </div>
        <div class="tracking-number" v-if="logisticsInfo.logisticsNo">
          <span class="label">快递单号：</span>
          <span class="value">{{ logisticsInfo.logisticsNo }}</span>
          <i class="el-icon-copy-document copy-icon" @click="handleCopyText(logisticsInfo.logisticsNo)" title="复制"></i>
        </div>
      </div>
      ...
      <div class="shipping-info">
        <div class="shipping-location">
          ...
          <span class="value">{{ (logisticsInfo.routeInfoVO && logisticsInfo.routeInfoVO.from && logisticsInfo.routeInfoVO.from.name) || '-' }}</span>
        </div>
      </div>
    </div>

    <!-- 物流状态时间线 -->
    <div class="timeline-section" v-if="logisticsInfo.detailInfoVOList && logisticsInfo.detailInfoVOList.length > 0">
      <div class="timeline-header">
        <i class="el-icon-time"></i>
        <span class="timeline-title">物流轨迹</span>
      </div>
      <div class="timeline-container" v-loading="loading" element-loading-text="正在获取物流信息...">
        <div 
...
import { getExpressInfoApi } from '@/api/wholesale';
...
export default {
  ...
  methods: {
    ...
    async getCompanyInfo() {
      ...
      this.loading = true;
      try {
        const res = await getExpressInfoApi({
