    ...
    <div class="procurement-dialog">
      ...
      <div class="form-section">
        ...
        <div class="form-row">
          ...
          <div class="form-item">
            ...
            <el-option v-for="item in skuList"
              ...
              :label="item.specName"
              :value="item.skuCode">
              <div @click="selectSku(item)">{{item.specName
               }}</div>
              </el-option>
            </el-select>
          </div>
        ...
        <div class="form-row">
          <div class="form-item">
            <label>sku</label>
            <el-input @blur="changeSkuCode" v-model="formData.skuCode" placeholder="请输入sku"></el-input>
          </div>
        </div>
      </div>
      <!-- 价格配置 -->
        ...
        <div class="price-config">
          ...
          <div class="price-row">
            ...
            <el-select v-model="formData.province" placeholder="请选择省份">
              <el-option v-for="item in provinceData" :key="item.districtId" :label="item.district" :value="item.districtId"></el-option>
            </el-select>
          </div>
            <div class="price-item">
              <label>账期类型</label>
              <el-select v-model="formData.accountingPeriod" placeholder="请选择账期类型">
      ...
      <div class="form-section">
        ...
        <div class="form-row">
          <div class="form-item">
            <label>发货时效</label>
            <el-select v-model="formData.deliveryTime" placeholder="请选择发货时效">
          ...
          <div class="form-item">
            <label>串码选项</label>
            <el-select v-model="formData.codeOptions" placeholder="请选择串码选项">
...
<script>
import {apiRuleSave,apiRuleUpdate} from "@/api/creatingOrders"
...
export default {
  ...
  methods: {
    ...
    handleConfirm() {
      ...
      if(this.isEdit){
        apiRuleUpdate(this.formData).then(res=>{
          if(res.code == 200){
            this.$emit('confirm')
            this.$emit('update:visible', false)
          }
        ...
        apiRuleSave(this.formData).then(res=>{
          if(res.code == 200){
            this.$emit('confirm')
            this.$emit('update:visible', false)
          }
    ...
    validateForm() {
      ...
      if( this.formData.priceLowest<=0||this.formData.priceLow<=0||this.formData.priceHign<=0||this.formData.priceHighest<=0){
      ...
      // 验证价格逻辑
      if (this.formData.priceLowest && this.formData.priceLow &&
          this.formData.priceLowest > this.formData.priceLow) {
        this.$message.error('价4不能大于价3')
        return false
      }

      if (this.formData.priceLow && this.formData.priceHign &&
          this.formData.priceLow > this.formData.priceHign) {
        this.$message.error('价3不能大于价2')
        return false
      }

      if (this.formData.priceHign && this.formData.priceHighest &&
