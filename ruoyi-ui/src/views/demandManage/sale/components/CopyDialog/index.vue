<template>
  ...
  >
    <div class="copy-dialog-content">
      ...
      <div class="info-item">
        <label>价格：</label>
        <span>{{ data.tradePrice == 0 ? '0元' : data.tradePrice ? data.tradePrice + '元' : '-'}}</span>
        <i
          v-if="data.tradePrice"
          class="el-icon-copy-document copy-icon"
          @click="copyText(data.tradePrice == 0 ? '0元' : data.tradePrice ? data.tradePrice + '元' : '-')"
        ></i>
      </div>
      <div class="info-item">
        <label>数量：</label>
