...
export default {
  ...
  mounted() {
    this.initColumnSearch(['payerName', 'tradeCompanyName', 'brand', 'productName'], { brand: { display: row => (row.brand || '-') + (row.category ? '/' + row.category : '') }, productName: { display: row => (row.productName || '-') + (row.skuName ? '/' + row.skuName : '') } })
    this.fetchOrderList()
  },
  methods: {
        // 发货时效显示方法
