    ...
    <el-dialog :visible.sync="planDialog.visible" width="1080px">
      ...
      <div class="plan-footer">
        ...
        <div class="plan-actions">
          <el-button size="small" @click="onPlanReset" :disabled="planDialog.loadingAll">重置</el-button>
          <el-button size="small" @click="onPlanSelectAll" :disabled="planDialog.loadingAll">全选</el-button>
...
export default {
  ...
  methods: {
    ...
    onPlanSelectAll() {
      ...
      for (let i = 0, len = rows.length; i < len; i++) {
        const row = rows[i];
        if (!row.isSummary && !row.isAbnormal) {
          const amount = Number(row.billingAmount) || 0;
          // 只选择金额大于0的行
          if (amount > 0) {
            row.selected = true;
    ...
    async onExportAging() {
      ...
      try {
        ...
        if (isBlob) {
          ...
          const supplierName = this.selectedSupplierName || '代发付款详情';
          const now = new Date();
          const timestamp =
            now.getFullYear() +
