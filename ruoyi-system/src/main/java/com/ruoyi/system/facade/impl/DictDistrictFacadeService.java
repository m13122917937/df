...
import com.ruoyi.system.manager.DictDistrictManager;
 ...
 */
@Service
public class DictDistrictFacadeService implements IDictDistrictFacade {

    @Autowired
    private DictDistrictManager dictDistrictManager;

    @Override
    public List<DictDistrictBO> list(DictDistrictQuery query) {
        DictDistrict domain = DictDistrictCov.INSTANCE.queryToDomain(query);
        return DictDistrictCov.INSTANCE.listToBO(dictDistrictManager.list(new QueryWrapper<>(domain)));
    }

    @Override
    public DictDistrictBO getOne(DictDistrictQuery query) {
        DictDistrict domain = DictDistrictCov.INSTANCE.queryToDomain(query);
        return DictDistrictCov.INSTANCE.toBO(dictDistrictManager.getOne(new QueryWrapper<>(domain)));
    }
    ...
    public boolean update(DictDistrictParam param, DictDistrictQuery query) {
        ...
        return dictDistrictManager.update(DictDistrictCov.INSTANCE.paramToDomain(param), new QueryWrapper<>(queryDomain));
    }
